...
if __name__ == '__main__':
    ...
    V = init_value(mdp)  # [36, ]
    policy = random_policy(mdp)  # [36, 4]

    # state value of 10
    # value = 0.0
    # orderedNames = [0, 1, 2, 3]
    # mat = np.squeeze(np.array([mdp.P[10][i] for i in orderedNames]))
    # print(mat)
    # mat[i], i = action
    #   p   s'  r  terminal
    # [[1.  4.  -1.  0.]
    ...
    #     orderedNames = [0, 1, 2, 3]
    #     mat = np.squeeze(np.array([mdp.P[state][i] for i in orderedNames]))
    #     for action in range(4):
    #         prob, next_state, reward, _ = tuple(mat[action])
    #         # prob = mat[action, 0]
    #         # next_state = int(mat[action, 1])
    #         # reward = mat[action, 2]
    #         V_new[state] += policy[state, action] * prob * (reward + discount * V[int(next_state)])
    #
    # print(np.reshape(V_new, (6, 6)))
    #
